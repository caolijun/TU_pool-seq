...
#
##special regions
pi_chr01 <- pi[pi$CHROM=="chr01",]
pi_chr01$CHROM <- factor(pi_chr01$CHROM, levels = unique(pi_chr01$CHROM))
pi_chr01 <- pi_chr01[pi_chr01[,3]>=3900000,]
pi_chr01 <- pi_chr01[pi_chr01[,3]<=5900000,]
str(pi_chr01)
min(pi_chr01$PBE)
max(pi_chr01$PBE)
...
#p <- ggplot(pi_chr01, aes(POS,PBE)) +
  ...
  #设定X轴
  scale_x_continuous(breaks = seq(3900000, 5900000, 1000000),labels = label_currency(scale_cut = cut_short_scale()),expand = c(0, 0),limits=c(3900000, 5900000)) +
  #去除绘图区和X轴之间的gap
  #scale_y_continuous(breaks = seq(0, 4500, 2000), labels = as.character(seq(0, 4500, 2000)), expand = c(0, 0),
...
pdf(paste(args[1],"_chr01_sdhB_ggplot3.pdf",sep=""),width = 5, height = 3)
p
dev.off()
